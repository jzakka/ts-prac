...
## 학습 규칙
1. **학습자가 직접 코드를 작성**: AI는 예제 코드만 제시하고, 학습자가 직접 타이핑하며 학습
2. 한 레슨에 하나의 폴더 (`src/lessonXX/index.ts`)
3. 각 파일 맨 위에 `export {};` 추가 (모듈로 만들어서 타입 충돌 방지)
4. Go와 비교하며 차이점 중심으로 이해
5. 이해 안 되면 바로 질문
6. 각 레슨 마지막에 실습 문제 1개 풀기
7. 다음 레슨 넘어가기 전에 현재 레슨 완전 이해

## AI 역할
- **해야 할 것**: 개념 설명, 예제 코드 제시, 질문 답변, Go와의 비교 설명, 레슨당 실습 문제 1개 제시
- **하지 말아야 할 것**: 파일 직접 작성, 전체 코드 완성본 제공
